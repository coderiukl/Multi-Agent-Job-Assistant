...
class ITviecScraper(BaseScraper):
    ...
    async def search(self, keyword: str, location: str, limit: int = 20) -> list[JobItem]:
        async with async_playwright() as p:
            ...
            try:
                ...
                logger.exception("ITviec scrape error: %s", e)
                return []
            finally:
                await browser.close()
    ...
    async def _scrape(self, browser: Browser, keyword: str, location: str, limit: int) -> list[JobItem]:
        ...
        # fallback-friendly URL
        url = f"{self.BASE_URL}?q={encoded_keyword}&l={loc_slug}"

        logger.info("[ITviec] keyword=%r", keyword)
        logger.info("[ITviec] url=%s", url)

        await page.goto(url, timeout=30000, wait_until="domcontentloaded")
        await page.wait_for_timeout(2500)

        cards = await page.query_selector_all(".job-card")
        logger.info("[ITviec] cards found: %s", len(cards))
        ...
        for card in cards[:limit]:
            try:
                ...
                location_el = await card.query_selector(".address")
                ...
                job_url = f"https://itviec.com{href}" if href.startswith("/") else href
                ...
                logger.warning("ITviec parse card error: %s", e)
                continue
    ...
    def _parse_salary(self, text: str) -> tuple[int | None, int | None]:
        ...
        if len(nums) >= 2:
            return int(nums[0]) * 1_000, int(nums[1]) * 1_000
        return None, None